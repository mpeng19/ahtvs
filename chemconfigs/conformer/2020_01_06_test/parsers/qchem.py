...
COORD_ENDING_MARKER_4 = "Point Group"  # Ending marker for qchem 4
COORD_ENDING_MARKER_5 = "Z-matrix Print:" # Ending marker for qchem 5
...
def parse_coords_starting_at_line(lines, start_line_num):
    ...
    line = lines[cur_line]
#    on_line_num = 0
    while COORD_ENDING_MARKER_5 not in line and COORD_ENDING_MARKER_4 not in line and line != '\n':
#        on_line_num += 1
#        print('Printing line')
#        print(line)
#        print('On line num',str(on_line_num))
...
def parse_interstate_strengths(lines, dictionary):
    ...
    while ENDING_MARKER not in line:
        ...
        if initial in dictionary:
            dictionary[int(initial)][int(final)] = {'strength':float(strength),'vector':[float(x),float(y),float(z)]}
        else:
            dictionary[int(initial)] = {int(final): {'strength':float(strength),'vector':[float(x),float(y),float(z)]}}
        cur_line += 1
        line = lines[cur_line]
